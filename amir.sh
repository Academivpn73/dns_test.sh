#!/usr/bin/env bash
# =======================================
# Game DNS Manager - Version 7.0 (Full)
...
title(){
  ...
  echo -e "$C"
  fast_line "╔══════════════════════════════════════════════════════════════════╗" 0.0006
  fast_line "║                        GAME DNS MANAGEMENT                        ║" 0.0005
  fast_line "╠══════════════════════════════════════════════════════════════════╣" 0.0005
  fast_line "║ Version: 7.0                                                     ║" 0.0004
  fast_line "║ Telegram: @Academi_vpn                                           ║" 0.0004
  fast_line "║ Admin:    @MahdiAGM0                                             ║" 0.0004
  fast_line "╚══════════════════════════════════════════════════════════════════╝" 0.0006
  echo -e "$RESET"
}
footer(){
  ...
  echo -e "$C"
  echo "==================================================================="
  echo " Version: 7.0  |  @Academi_vpn  |  @MahdiAGM0"
  echo "==================================================================="
  echo -e "$RESET"
}
...
measure_ms(){
  # Try ping/ping6; if not available or blocked, return fallback
  local ip="$1" out
  if [[ "$ip" == *:* ]]; then
...
normalize_game(){
  # unify names for lookup (lowercase + trim)
  echo "$1" | tr '[:upper:]' '[:lower:]' | sed 's/  */ /g' | sed 's/^ *//;s/ *$//'
}

# ---------------- Banks (big) ----------------
# Global/anti-censorship IPv4 (200+)
ANTI_V4=(
# Cloudflare public & malware/ad variants
...
# ControlD
76.76.19.19 76.223.122.150 76.76.2.0 76.76.10.0 76.76.2.1 76.76.10.1
# Verisign
64.6.64.6 64.6.65.6
# Comodo
...
# OpenNIC/Uncensored / misc well-known
...
208.67.222.220 208.67.220.222 208.67.220.2 208.67.222.2
1.1.1.1 1.0.0.1 8.8.8.8 8.8.4.4 9.9.9.9
)

# Regional IPv4 (Iran / UAE / Saudi / Turkey) — curated ranges + resolvers
IR_V4=(
178.22.122.100 178.22.122.101 185.51.200.2 185.51.200.4
5.200.200.200 46.245.69.2 46.245.69.3 217.218.127.127
31.7.64.1 31.7.64.2 62.201.220.50 62.201.220.51
85.185.39.10 85.185.39.11 185.55.226.26 185.55.225.26
217.11.16.21 217.11.16.22 185.83.114.56 185.117.118.20
)
AE_V4=(
...
81.212.65.50 81.212.65.51
212.156.4.1 212.156.4.2
85.111.3.3 85.111.3.4
176.43.1.1 176.43.1.2 176.43.1.3
88.255.168.248 88.255.168.249
213.14.227.118 213.14.227.119
)

# IPv6 samples (global/regional)
...
# Download / anti-block (100+ curated)
DOWNLOAD_V4=(
...
76.76.19.19 76.223.122.150 76.76.2.0 76.76.10.0 76.76.2.1 76.76.10.1
45.90.28.0 45.90.30.0 45.90.28.10 45.90.30.10 45.90.28.169 45.90.30.169
...
45.90.28.226 45.90.30.226 45.90.28.193 45.90.30.193
77.88.8.8 77.88.8.1 77.88.8.2 77.88.8.88
...
# Sensitive/blocked games in IR → anti-censorship DNS
blocked_in_ir=(
...
# Optional per-game biases (seed list to mix early for better odds)
declare -A GAME_SEED
...
is_blocked_in_ir(){ local n="$1" g; for g in "${blocked_in_ir[@]}"; do [[ "$g" == "$n" ]] && return 0; done; return 1; }
...
pick_best_two(){
  ...
  # Prefer sub-50ms if possible; then sub-80; then sub-120; else lowest
  local thr
...
serve_game_dns(){
  ...
  if is_blocked_in_ir "$game"; then
    pool+=( "${ANTI_V4[@]}" )   # anti-censorship for blocked titles
  else
    candidates_for_game "$game" 4
    pool+=( "${CAND[@]}" )
    pool+=( "${IR_V4[@]}" )     # local/regional mix for latency
    pool+=( "${ANTI_V4[@]}" )
  fi
  # Failsafe
...
serve_search_dns(){
  local g="$1" dev="$2"
  # Try to match against known lists; if not found, use generic anti + region
  local normalized="$(normalize_game "$g")" name_match=""
  local x
  for x in "${mobile_games[@]}"; do
    [[ "$(normalize_game "$x")" == "$normalized" ]] && { name_match="$x"; break; }
  done
  ...
  if [[ -n "$name_match" ]]; then
    ...
    # Unlisted game: generic smart pool
...
serve_generator(){
  echo "DNS Generator (country → IP version → count)"
  echo " 1) Iran"
  echo " 2) UAE"
  echo " 3) Saudi Arabia"
  echo " 4) Turkey"
  ...
  # Rank by ping (always produce output)
  local pairs=() ip ms
...
menu_mobile(){
  ...
  echo "Mobile Games:"
  local i=1
  for g in "${mobile_games[@]}"; do printf "%2d) %s\n" "$i" "$g"; i=$((i+1)); done
  echo
...
menu_pc(){
  ...
  local i=1
  for g in "${pc_console_games[@]}"; do printf "%2d) %s\n" "$i" "$g"; i=$((i+1)); done
  echo
  read -rp "Pick game number: " n
  [[ "$n" =~ ^[0-9]+$ ]] || { echo "Invalid."; pause_enter; return; }
